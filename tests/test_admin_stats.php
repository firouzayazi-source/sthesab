...
try {
    ...
    // ⛔ این کاربر برای «تراکنشِ امروز» لازم است و بدونِ او تست **کور**
    //    می‌ماند: بقیه‌ی fixture ها تراکنشِ امروز دارند، پس جهشِ
    //    «today_tx را همان tx بگذار» روی همه‌شان سبز می‌ماند. اینجا
    //    عمداً بیشترِ تراکنش‌ها عقب‌اند و فقط یکی امروز است.
    $uOld    = $mk('__stat_oldtx__');
    ...
    foreach ([$uTx, $uCheque, $uIdle, $uEmpty, $uRef, $uOld] as $uid) { $addWallet($uid, 'کیف پول'); }
    ...
    $txStmt->execute(['u' => $uOld]);   // یک تراکنشِ امروز

    // ⚠ `created_at` عمداً عقب برده می‌شود، نه `transaction_date`: «امروز»
    //   در این صفحه یعنی «امروز ثبت شده»، همان معنایی که `days_since` و
    //   «آخرین فعالیت» هم دارند. کاربر می‌تواند امروز خرجِ ماهِ پیش را
    //   ثبت کند و آن **فعالیتِ امروز** است.
    $oldStmt = $pdo->prepare('INSERT INTO transactions (user_id, type, amount, title, transaction_date, created_at)
                              VALUES (:u, "expense", 1000, "تستِ قدیمی", CURDATE(),
                                      DATE_SUB(NOW(), INTERVAL 3 DAY))');
    for ($i = 0; $i < 3; $i++) { $oldStmt->execute(['u' => $uOld]); }
    ...
    // ---------------------------------------------------------------
    T::group('⛔ «تراکنشِ امروز» جدا از جمعِ کل');

    // ⛔ خرابیِ گزارش‌شده، برای بارِ دوم: مالکِ نصب گفت «مصطفی امروز ۱
    //    تراکنش داشته اما زده ۷ تا». عددِ صفحه درست بود ولی **جمعِ کلِ
    //    تاریخچه** را می‌گفت و او آن را «امروز» خواند. حالا هر دو عدد
    //    صریح‌اند و این گروه می‌سنجد که واقعاً دو چیزِ متفاوت‌اند.
    $r = $row('__stat_oldtx__');
    T::ok($r !== null, 'کاربرِ تراکنشِ قدیمی در خروجی هست');
    T::same(4, (int)$r['tx'], 'جمعِ کلِ تراکنش‌هایش چهار است');
    T::same(1, (int)$r['today_tx'],
            '⛔ ولی امروز فقط **یکی** ثبت کرده — عددِ کل «امروز» نیست');
    T::same(4, (int)$r['records'], 'و «رکورد» هم همان چهار می‌ماند');

    $r = $row('__stat_tx__');
    T::same(6, (int)$r['tx'], 'کاربرِ امروزی شش تراکنش دارد');
    T::same(6, (int)$r['today_tx'], 'و هر شش را امروز ثبت کرده');

    // ⛔ و سمتِ دیگرش: کسی که امروز تراکنشی ثبت نکرده باید **صفر**
    //    بگیرد، نه چیزِ دیگری. بدونِ این بررسی، جهشِ «همیشه ۰» فقط از
    //    یک طرف گرفته می‌شد.
    T::same(0, (int)$row('__stat_idle__')['today_tx'],
            'کاربرِ کم‌فعال امروز صفر تراکنش دارد');
    T::same(0, (int)$row('__stat_cheque__')['today_tx'],
            'کاربری که فقط چک و طلبِ امروز دارد هم صفر — چک تراکنش نیست');
    T::same(0, (int)$row('__stat_empty__')['today_tx'],
            'کاربرِ خالی هم صفر، نه null');

/**
 * اجرای `window.kbNeedsKeyboard()` در node، برای تستِ PHP.
 *
 ...
 *
 * ورودی: JSON آرایه‌ای از عنصرهای ساختگی روی stdin، هر کدام
 *        `{tagName, type, isContentEditable}` یا `null`.
...
if (typeof sandbox.window.kbNeedsKeyboard !== 'function') {
    console.error('ERR_MISSING:kbNeedsKeyboard');
    process.exit(3);
}
...
process.stdin.on('end', () => {
    ...
    const out = items.map((el) => {
        try { return sandbox.window.kbNeedsKeyboard(el) === true; }

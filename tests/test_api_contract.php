...
// ⛔ قاعده ۳۲ — منوی <select> را خودمان رندر می‌کنیم.
//
// **خرابیِ واقعی و فقط روی گوشی:** منوی بازشده‌ی `<select>` روی iOS
// کرومِ خودِ سیستم است و جهتش را از **زبانِ گوشی** می‌گیرد، نه از
// `dir="rtl"` صفحه. با اندازه‌گیریِ اسکرین‌شات ثابت شد (لبه‌ی چپِ همه‌ی
// ردیف‌ها روی یک عدد، لبه‌ی راست ناهموار، و تیک سمتِ چپ). پیش از این
// `select, select option { direction: rtl; text-align: right }` امتحان
// شد و **جواب نداد**، چون هیچ CSS ای به آن منو نمی‌رسد.
//
// ⚠ سه چیز اینجا بی‌صدا می‌شکند و هیچ‌کدام روی دسکتاپ دیده نمی‌شود:
//   ۱. افتادنِ `touchend` — روی iOS منوی بومی برمی‌گردد و همه چیز در
//      کرومِ دسکتاپ سالم به نظر می‌رسد (همان درسِ منوی کناری، آینه‌وار).
//   ۲. نفرستادنِ `change` — `setType()` و `resetSelect()` و
//      `syncPersonPicker()` بی‌صدا از کار می‌افتند.
//   ۳. نبودِ بلوکِ دسکتاپ — شیت روی عرضِ ۱۴۰۰ پیکسل یک نوارِ درازِ
//      لبه‌ی پایین می‌شود؛ همان چیزی که یک بار سرِ `.sheet` گفته شد.
//      اندازه‌گیری شد: بدونِ آن بلوک عرضش ۱۳۸۵ پیکسل درمی‌آمد.
T::group('قاعده ۳۲ — انتخابگرِ گزینه‌ها به‌جای منوی بومی');

$badPick = [];

foreach (['.opt-overlay', '.opt-sheet'] as $sel) {
    if ($ruleBody($css, $sel) === null) {
        $badPick[] = "قاعده‌ی {$sel} در style.css نیست — انتخابگر برداشته شده"
                   . ' و منوی بومیِ iOS برمی‌گردد';
    }
}

$itemBody = $ruleBody($css, '.opt-item');
if ($itemBody === null || !preg_match('/text-align\s*:\s*right/', $itemBody)) {
    // ⚠ فیزیکی، نه `start`: مقدارِ منطقی نسبت به جهتِ پایه دوباره حل
    //   می‌شود و همان چیزی است که در منوی بومی غلط از آب درمی‌آمد.
    $badPick[] = '.opt-item دیگر `text-align: right` ندارد؛ گزینه‌ها دوباره'
               . ' چپ‌چین می‌شوند';
}

// `$desktop` از قاعده ۳۱ می‌آید و همه‌ی بلوک‌های `min-width: 901px` را دارد.
if ($desktop === '' || !preg_match('/\.opt-overlay\s*\{[^}]*align-items\s*:\s*center/', $desktop)) {
    $badPick[] = '`.opt-overlay` در بلوکِ دسکتاپ وسط‌چین نمی‌شود؛ روی عرضِ بزرگ'
               . ' یک نوارِ درازِ لبه‌ی پایین می‌شود';
}
if ($desktop === '' || !preg_match('/\.opt-sheet\s*\{[^}]*max-width/', $desktop)) {
    $badPick[] = '`.opt-sheet` در بلوکِ دسکتاپ `max-width` ندارد؛ عرضش به کلِ'
               . ' پنجره کشیده می‌شود';
}

// و نیمه‌ی جاوااسکریپت. بلوکِ انتخابگر با تطبیقِ آکولاد جدا می‌شود تا
// این بررسی به بقیه‌ی فایل سرریز نکند — همان درسِ قاعده ۲۹.
$appJs = (string)@file_get_contents(__DIR__ . '/../assets/js/app.js');
$appNoCom = preg_replace('#/\*.*?\*/#s', '', $appJs);
$appNoCom = preg_replace('#//[^\n]*#', '', (string)$appNoCom);
$pick = '';
if (preg_match('/\(function optionPicker\s*\(\s*\)\s*\{/', (string)$appNoCom, $m, PREG_OFFSET_CAPTURE)) {
    $start = $m[0][1] + strlen($m[0][0]);
    $depth = 1; $len = strlen($appNoCom);
    for ($i = $start; $i < $len && $depth > 0; $i++) {
        if ($appNoCom[$i] === '{') { $depth++; }
        elseif ($appNoCom[$i] === '}') { $depth--; }
    }
    $pick = substr($appNoCom, $start, $i - $start - 1);
}
if ($pick === '') {
    $badPick[] = 'optionPicker() در app.js نیست — قاعده ۳۲ کور شده';
} else {
    if (!preg_match('/addEventListener\s*\(\s*[\'"]touchend[\'"]/', $pick)) {
        $badPick[] = 'انتخابگر دیگر `touchend` را نمی‌گیرد؛ روی iOS منوی بومی'
                   . ' برمی‌گردد و در کرومِ دسکتاپ هیچ نشانه‌ای ندارد';
    }
    if (!preg_match('/addEventListener\s*\(\s*[\'"]mousedown[\'"]/', $pick)) {
        $badPick[] = 'انتخابگر دیگر `mousedown` را نمی‌گیرد؛ روی ماوس و دسکتاپ'
                   . ' منوی بومی باز می‌شود';
    }
    if (!preg_match('/passive\s*:\s*false/', $pick)) {
        $badPick[] = '`passive: false` روی touchend نیست؛ `preventDefault` نادیده'
                   . ' می‌ماند و منوی بومی هم باز می‌شود';
    }
    if (!preg_match('/dispatchEvent\s*\(\s*new Event\s*\(\s*[\'"]change[\'"]/', $pick)) {
        $badPick[] = 'انتخابگر رویدادِ `change` نمی‌فرستد؛ setType() و'
                   . ' resetSelect() و syncPersonPicker() بی‌صدا از کار می‌افتند';
    }
    if (!preg_match('/\.selectedIndex\s*=/', $pick)) {
        $badPick[] = 'انتخابگر روی خودِ <select> نمی‌نویسد؛ مقدارِ فرم عوض نمی‌شود';
    }
}

T::bulk(10, $badPick, '⛔ انتخابگرِ گزینه‌ها سرِ جایش است و روی هر دو مسیر می‌گیرد');

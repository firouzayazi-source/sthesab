...
<div class="card">
     ...
     *
     * ⛔ و همین کافی نبود، چون **مبنای** دو عدد فرق می‌کند:
     * `active_cost` بهای **خرید** است ولی `sold_total` قیمت **فروش**
     * (`SUM(salePrice)` در `ownerTotals()` سمتِ فروشگاه). مالکِ نصب
     * ۳۱۳ + ۱۴۵ را جمع می‌زد و با ۵۲۷ نمی‌خواند، در حالی که حسابِ
     * واقعیِ هشدارِ پایین با بهای **خریدِ** همان دستگاهِ فروخته‌شده
     * است. یعنی عددِ اختلاف از روی هیچ‌کدام از عددهای کارت درنمی‌آمد —
     * همان درسِ «۱ تراکنش · ۷ رکورد» در آمار استفاده: عددها درست،
     * برچسب‌ها نه.
     *
     * پس دو کار: «فروخته‌شده» صریح می‌گوید قیمتِ فروش است، و
     * **«سرمایه در کالا»** (همان `$expectCapital`) به‌عنوان یک قلمِ
     * دیدنی می‌نشیند تا «اصل سرمایه منهای آن» دقیقاً همان عددی باشد
     * که هشدار می‌گوید. محاسبه به همین دلیل **پیش از** رندرِ جمع‌ها
     * آمده، نه بعدش.
     */

    /*
     * ⛔ این هشدار فقط در حالتی داده می‌شود که رابطه‌اش **اثبات‌پذیر**
     * است. دو حالت اثبات‌پذیرند و هیچ‌کدام حدس نمی‌زند:
     *
     *   ۱. هیچ دستگاهی فروخته نشده → اصلِ سرمایه باید دقیقاً برابرِ
     *      بهای خریدِ کالاهای در انبار باشد.
     *   ۲. فروش انجام شده ولی **بهای خریدِ همان دستگاه‌ها در پاسخ هست**
     *      (هر ردیف `cost` دارد، چه فروخته چه نه) → سرمایه باید برابرِ
     *      جمعِ بهای خریدِ **همه‌ی** دستگاه‌ها باشد، چون سرمایه‌ی
     *      دستگاهِ فروخته‌شده سرِ جایش می‌ماند و فقط از انبار رفته.
     *
     * ⛔ و حالتِ ۲ **پیش‌شرطِ خودش را می‌سنجد، فرض نمی‌کند**: اگر پاسخ
     *    دستگاه‌های فروخته‌شده را اصلاً نفرستد (یا فهرست بریده باشد)،
     *    جمعِ ردیف‌ها ناقص است و هشدارِ **الکیِ** «مازاد» می‌ساخت — که
     *    از نبودِ سنجش بدتر است. پس فقط وقتی حساب می‌شود که تعدادِ
     *    ردیف‌های فروخته با `sold_count` یکی باشد؛ وگرنه ساکت می‌ماند.
     *
     * ⚠ حدش صادقانه: لوازمِ جانبیِ **فروخته‌شده** در پاسخ ردیف ندارند،
     *   پس اگر کسی لوازم جانبی فروخته باشد هر دو حالت «مازاد» نشان
     *   می‌دهند. این محدودیت از قبل هم بود و بدتر نشد.
     */
    $devCost  = 0;
    $soldCost = 0;
    $soldRows = 0;
    foreach ($own['rows'] as $r) {
        ...
        $devCost += (int)$r['cost'];
        if ($r['sold']) { $soldRows++; $soldCost += (int)$r['cost']; }
    }
    $expectCapital = null;
    if ($own['capital'] !== null) {
        if ($own['sold_count'] === 0) {
            $expectCapital = $own['active_cost'] + $own['items_cost'];
        } elseif (!$own['capped'] && $soldRows === $own['sold_count']) {
            $expectCapital = $devCost + $own['items_cost'];
        }
    }
    /*
     * ⛔ و علامتِ اختلاف دو معنای کاملاً متفاوت دارد، پس دو پیامِ جدا:
     *   سرمایه **کمتر** از کالا → خریدی سند نخورده (خرابیِ دفتر، کهربایی).
     *   سرمایه **بیشتر** از کالا → پولی که هنوز به کالا تبدیل نشده، که
    ...
    <div class="store-own-stats">
        ...
        <div class="store-own-stat">
            ...
            <small><?= toPersianDigits((string)$own['sold_count']) ?> دستگاه · قیمت فروش</small>
        </div>
        <?php if ($expectCapital !== null): ?>
        <div class="store-own-stat">
            <span class="store-own-label">سرمایه در کالا</span>
            <b class="ltr-num"><?= formatMoney($expectCapital) ?></b>
            <small>
                بهای خرید<?php if ($soldCost > 0): ?> —
                با <?= formatMoney($soldCost) ?> بابتِ فروخته‌شده‌ها<?php endif; ?>
            </small>
        </div>
        <?php endif; ?>
        <?php if (!$own['is_house']): ?>
    ...
    <?php if ($capGap !== null && $capGap < 0): ?>
        <p class="hint" style="color:var(--warn-ink);">
            اصل سرمایه <?= formatMoney(-$capGap) ?> تومان از «سرمایه در کالا»
            <b>کمتر</b> است. یعنی خریدِ دست‌کم یک کالا به
    ...
    <?php elseif ($capGap !== null && $capGap > 0): ?>
        <p class="hint">
            «اصل سرمایه» منهای «سرمایه در کالا» می‌شود
            <?= formatMoney($capGap) ?> تومان — یعنی این مقدار از سرمایه‌ی
            ثبت‌شده هنوز به کالا تبدیل نشده است. اگر انتظار دارید همه‌ی
            سرمایه خرج شده باشد، در پنل فروشگاه سندهای سرمایه‌ی این شخص را
            بررسی کنید (مبلغِ اشتباه یا سندِ تکراری).
        </p>
    <?php endif; ?>

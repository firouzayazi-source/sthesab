...
[$todayJy, $todayJm, $todayJd] = gregorianToJalali(
    (int)date('Y', strtotime($todayStr)),
    (int)date('m', strtotime($todayStr)),
    (int)date('d', strtotime($todayStr))
);
...
$pageTitle = 'تقویم مالی';
include __DIR__ . '/includes/header.php';
?>

<div class="card">
    <div class="cal-header">
        <a href="?jy=<?= $prevJy ?>&jm=<?= $prevJm ?>" class="cal-nav" aria-label="ماه قبل">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
        </a>
        <div class="cal-title">
            <?= h($monthNames[$jm]) ?> <?= toPersianDigits($jy) ?>
            <?php if ($jy !== $todayJy || $jm !== $todayJm): ?>
                <a href="calendar.php" class="cal-today-link">برو به امروز</a>
            <?php endif; ?>
        </div>
        <a href="?jy=<?= $nextJy ?>&jm=<?= $nextJm ?>" class="cal-nav" aria-label="ماه بعد">

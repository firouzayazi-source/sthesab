...
class Notify
{
    ...
    public static function generateFor(int $userId, bool $force = false): int
    {
        ...
        $made += self::generateAdminErrors($userId, $today);
        return $made;
    }

    /**
     * ⛔ «اپ خراب است» باید به مالکِ نصب برسد، نه اینکه منتظرِ نگاهش بماند.
     *
     * **خواسته‌ی مالکِ نصب:** «توی بخش مدیریت اطلاع‌رسانی بشه.» نشانِ
     * نوارِ `admin/` نیمه‌ی اول است و فقط وقتی دیده می‌شود که پنل باز
     * شود؛ این نیمه‌ی دوم است و در مرکزِ اعلانِ همان صفحه‌ی خانه می‌نشیند.
     *
     * ⛔ **فقط برای مدیر.** کاربر عادی نباید بداند کدِ اپ کجا شکسته —
     *    نه به او مربوط است نه کاری از دستش برمی‌آید.
     *
     * ⛔ **و فقط برای خطایی که در ۲۴ ساعت گذشته واقعاً رخ داده**
     *    (`AppErrors::openSince()`)، نه هر خطای باز. تفاوتش همان چیزی
     *    است که «هشدارِ همیشگی» را می‌سازد: خطایی که مالکِ نصب عمداً باز
     *    گذاشته و ماه‌ها است رخ نداده نباید هر روز یک اعلان بدهد —
     *    وگرنه آدم را عادت می‌دهد اعلان‌ها را نادیده بگیرد و آن‌وقت
     *    اعلانِ خطای واقعی هم دیده نمی‌شود.
     *
     * ⛔ **`dedup_key` روزانه است، نه به‌ازای هر خطا.** با کلیدِ
     *    فینگرپرینتی، یک روزِ بد بیست اعلان می‌ساخت — دقیقاً همان
     *    شلوغی‌ای که این کار برای رفعش انجام شد. عدد داخلِ کلید هم
     *    نمی‌آید، وگرنه هر بار که شمارش عوض می‌شد یک اعلانِ تازه بود.
     */
    private static function generateAdminErrors(int $userId, string $today): int
    {
        if (($_SESSION['role'] ?? '') !== 'admin') { return 0; }
        if (!class_exists('AppErrors')) { return 0; }

        $n = AppErrors::openSince(24);
        if ($n < 1) { return 0; }

        return self::push(
            $userId,
            'apperr',
            $n === 1 ? 'یک خطای رسیدگی‌نشده در برنامه' : 'خطای رسیدگی‌نشده در برنامه',
            'در ۲۴ ساعت گذشته ' . toPersianDigits((string)$n)
                . ' خطای متمایز رخ داده که هنوز رسیدگی نشده است. '
                . 'در «مدیریت ← خطاها» می‌بینید چه چیزی و کجا، و همان‌جا «برطرف شد» می‌زنید.',
            'admin/errors.php',
            'apperr:' . $today
        ) ? 1 : 0;
    }

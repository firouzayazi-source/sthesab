...
public class BankSmsReceiver extends BroadcastReceiver {
    ...
    /**
     * ⛔ «تا اینجا به سایت داده شد» — نشانه‌ی آوردنِ صندوقِ پیامک.
     *
     *    `HesabLauncherActivity` با هر باز شدنِ اپ پیامک‌های بانکیِ بعد از
     *    این نشانه را در `#smsq=` به سایت می‌دهد و فقط **همان‌جا** جلویش
     *    می‌برد. گیرنده به آن دست نمی‌زند: اعلانی که زده نشده، پیامکی
     *    نیست که به دفتر رسیده باشد.
     */
    ...
    public void onReceive(Context ctx, Intent intent) {
        ...
        //    همین پیامک باید از صندوق به دفتر برسد
        //    (`HesabLauncherActivity`). نسخه‌ی قبلی اینجا نشانه را جلو
    ...
    static void postNotification(Context ctx, String sender, String text) {
        ...
        try {
            ...
            // ⚠ `URLEncoder` فاصله را `+` می‌کند و `decodeURIComponent`ِ سایت آن را
            //   به فاصله برنمی‌گرداند؛ متن با `+` به‌جای فاصله به `parseBankSms()`
            //   می‌رسید. پس `%20`.
            url = ctx.getString(R.string.launch_url) + "#sms="

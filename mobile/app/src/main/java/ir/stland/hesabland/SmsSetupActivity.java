 ...
 *    هم بود.
 *
 * ⛔ **پیش‌فرض روشن است** (`BankSmsReceiver.DEFAULT_ON`) — و این یک بار
 *    برعکس بود. خواستِ مالکِ نصب: «پیش‌فرض نوتیف و اطلاع‌رسانی و تمام
 *    قابلیت‌ها باید فعال باشد». با پیش‌فرضِ خاموش، کسی که فقط اپ را نصب
 *    و باز می‌کرد هرگز پیامکی نمی‌گرفت؛ حالا مجوز در اولین اجرا پرسیده
 *    می‌شود (`HesabLauncherActivity`) و این صفحه برای **خاموش کردن** و
 *    رفعِ خرابی است. رد کردنِ مجوز هیچ دری را نمی‌بندد و اپ کاملاً باز
 *    می‌شود.
 *
...
public class SmsSetupActivity extends AppCompatActivity {
    ...
    private boolean enabled() {
        return prefs().getBoolean(BankSmsReceiver.PREF_ON, BankSmsReceiver.DEFAULT_ON);
    }
    ...
    private String lastEventLine() {
        ...
        if (at <= 0L) {
            ...
            //    نفیِ آن نوشته شد.
            //
            // ⚠ کلید پیش‌فرض روشن است، پس این شاخه یعنی یا کاربر خودش
            //   خاموشش کرده یا مجوزِ پیامک داده نشده.
    ...
    public void onRequestPermissionsResult(int req, String[] perms, int[] results) {
        ...
        if (req == REQ_SMS) {
            // ⛔ رد کردن هیچ چیزی را نمی‌شکند: پیامی داده می‌شود و اپ عادی
            //    جلو می‌رود.
            // ⚠ و کلید را **خاموش نمی‌کند**. «مجوز نداد» با «نمی‌خواهم»
            //   یکی نیست؛ اگر اینجا `false` نوشته می‌شد، دیالوگِ اولین
            //   اجرا (که فقط برای کلیدِ روشن می‌پرسد) دیگر هرگز درباره‌ی
            //   پیامک نمی‌پرسید. حالتِ «روشن ولی بی‌مجوز» خودش پیامِ
            //   روشن و دکمه‌ی رفع دارد.
            Toast.makeText(this, R.string.sms_perm_denied, Toast.LENGTH_LONG).show();

...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ---------- انتخابگرِ گزینه‌ها: جای منوی بومیِ <select> ----------
       ⛔ خودِ عنصرِ <select> سرِ جایش می‌ماند و هیچ‌کجای اپ عوض نمی‌شود:
       `name`، `value`، فهرستِ `options`، رویدادِ `change` و ارسالِ فرم
       همه دست‌نخورده‌اند، پس `setType()`، `resetSelect()`،
       `syncPersonPicker()` و `smsAutoOk()` که همه به وجودِ خودِ
       `<select>` بندند بی‌تغییر کار می‌کنند. تنها چیزی که عوض می‌شود
       این است که **باز شدنِ** منو به جعبه‌ی خودمان می‌رود.
       دلیلش: منوی بازشده‌ی `<select>` روی iOS کرومِ خودِ سیستم است و
       جهتش را از زبانِ گوشی می‌گیرد نه از `dir="rtl"` صفحه — با
       اندازه‌گیریِ اسکرین‌شات ثابت شد (لبه‌ی چپِ همه‌ی ردیف‌ها روی یک
       عدد و لبه‌ی راست ناهموار). `direction`/`text-align` روی خودِ
       عنصر هم به آن نمی‌رسید. توضیح کامل در CLAUDE.md.
       ⛔ برای همه‌ی دستگاه‌ها یکی است، نه فقط iOS — همان قاعده‌ی
       `.switch` که چک‌باکسِ خامِ مرورگر را همه‌جا کنار گذاشت. دو رفتارِ
       جدا دیر یا زود از هم دور می‌افتند. */
    (function optionPicker() {
        var overlay = null, sheet = null, current = null, lastTouch = 0;
        ...
        // برچسبِ فیلد بالای جعبه می‌آید، وگرنه کاربر فهرستی از نام‌ها
        // می‌بیند بی‌آنکه بداند کدام فیلد را دارد پر می‌کند.
        function labelFor(sel) {
            ...
            if (!lab) {
                var box = sel.closest('.form-group, .form-row > div, label');
                if (box) lab = box.querySelector('label');
            }
        ...
        function close() {
            ...
            var el = current;
            current = null;
            // فوکوس برمی‌گردد چون `preventDefault` روی تپ آن را نگرفته
            // بود؛ بدونش کاربرِ کیبورد جای خودش را گم می‌کند.
        ...
        function pick(sel, idx) {
            ...
            close();
            // ⛔ فقط وقتی `change` می‌دهیم که واقعاً عوض شده باشد —
            // همان قراردادی که خودِ مرورگر دارد.
        ...
        function open(sel) {
            ...
            Array.prototype.forEach.call(sel.options, function (opt, i) {
                ...
                if (opt.disabled || (group && group.disabled)) btn.disabled = true;

                var tick = document.createElement('span');
                tick.className = 'opt-check';
                tick.textContent = on ? '✓' : '';
                var txt = document.createElement('span');
                txt.className = 'opt-text';
                // ⚠ `textContent` نه `innerHTML`: نامِ دسته و شخص را
                // خودِ کاربر وارد می‌کند.
                txt.textContent = opt.text;
                btn.appendChild(tick);
                btn.appendChild(txt);
                btn.addEventListener('click', function () { pick(sel, i); });
            ...
            lockBodyScroll();
            sheet.scrollTop = 0;
            var cur = sheet.querySelector('[aria-selected="true"]');
            if (cur && cur.scrollIntoView) cur.scrollIntoView({ block: 'nearest' });
        }

        function eligible(el) {
            return el && el.tagName === 'SELECT' && !el.disabled
                && !el.multiple && (el.size || 1) <= 1 && el.options.length > 0;
        }

        // ⛔ هر دو مسیر لازم است و دلیلش فرق می‌کند:
        //   • `touchend` — روی iOS منوی بومی از همان تپ باز می‌شود؛
        //     `preventDefault` اینجا هم فوکوس را می‌گیرد هم `click`ِ
        //     ساختگی را، پس منو اصلاً فرصت باز شدن پیدا نمی‌کند.
        //   • `mousedown` — مسیرِ دسکتاپ و ماوس، که اصلاً `touchend`
        //     ندارد.
        // نگهبانِ زمانی جلوی باز شدنِ دوباره از رویدادِ ساختگیِ بعدی را
        // می‌گیرد. هر دو در فازِ capture اند تا هیچ هندلرِ صفحه‌ای
        // جلوتر از ما تصمیم نگیرد.
        ...
        document.addEventListener('keydown', function (e) {
            if (current && (e.key === 'Escape' || e.key === 'Esc')) close();
        });
    })();
